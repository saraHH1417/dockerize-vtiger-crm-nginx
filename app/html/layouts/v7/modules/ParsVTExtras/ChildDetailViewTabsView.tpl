...
{strip}
    ...
    <div class="block block_{$BLABEL}">
    {/if}
        {assign var=BLOCK value=$BLOCK_LIST[$BLOCK_LABEL_KEY]}
        {assign var=IS_HIDDEN value=$BLOCK->isHidden()}
        {assign var=WIDTHTYPE value=$USER_MODEL->get('rowheight')}
        {assign var=BLOCKID value=$BLOCK_LIST[$BLOCK_LABEL_KEY]->get('id')}
        <input type=hidden name="timeFormatOptions" data-value='{$DAY_STARTS}' />
        <div class="block-child" data-id="{$BLOCKID}">
            <h4 class="textOverflowEllipsis maxWidth50">
                <img class="cursorPointer alignMiddle {$BLOCKTOGGLE} {if !($IS_HIDDEN)} hide {/if} "  src="{vimage_path('arrowRight.png')}" data-mode="hide" data-id={$BLOCKID}>
                <img class="cursorPointer alignMiddle {$BLOCKTOGGLE} {if ($IS_HIDDEN)} hide {/if}"  src="{vimage_path('arrowdown.png')}" data-mode="show" data-id={$BLOCKID}>&nbsp;
