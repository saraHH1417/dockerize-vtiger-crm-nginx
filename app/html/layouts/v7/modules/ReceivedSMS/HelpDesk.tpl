...
<input data-link="true" value="&amp;sub=(Subject)" type="checkbox" class="inputElement" style="width:15px;height:15px;">
</span></td>
<td class="fieldLabel medium"><label class="muted pull-right marginRight10px">{vtranslate('Status', $MODULE)}</label></td>
<td class="fieldValue medium"><span class="span10">
<select data-link="true"  data-fieldtype="picklist" class="inputElement select2  select2-offscreen" type="picklist" style="width:200px"><option></option>{$Listticketstatus}</select>
</span></td>
</tr>
<tr>
<td class="fieldLabel medium"><label class="muted pull-right marginRight10px">{vtranslate('Category', 'HelpDesk')}</label></td>
<td class="fieldValue medium"><span class="span10">
<select data-link="true" data-fieldtype="picklist" class="inputElement select2  select2-offscreen" type="picklist" style="width:200px"><option></option>{$Listcategories}</select>
</span></td>
<td class="fieldLabel medium"><label class="muted pull-right marginRight10px">{vtranslate('Priority', 'HelpDesk')}</label></td>
<td class="fieldValue medium"><span class="span10">
<select data-link="true" data-fieldtype="picklist" class="inputElement select2  select2-offscreen" type="picklist" style="width:200px"><option></option>{$Listpriorities}</select>
</span></td>
</tr>
<tr>
<td class="fieldLabel medium"><label class="muted pull-right marginRight10px">{vtranslate('LBL_SEVERITY', 'HelpDesk')}</label></td>
<td class="fieldValue medium"><span class="span10">
<select data-link="true" data-fieldtype="picklist" class="inputElement select2  select2-offscreen" type="picklist" style="width:200px"><option></option>{$Listseverities}</select>
</span></td>
...
<optgroup label="{vtranslate('Groups', $MODULE)}">{str_replace("assignto","tassignto",$ListGroups)}</optgroup>
            ...
            <tr><td colspan="4"><textarea id="myCode" name="update" cols="85" rows="4" STYLE="direction:ltr;" class="form-control" READONLY>{vglobal('site_URL')}modules/ReceivedSMS/capture.php?type=helpdesk&amp;key={$sms_key}&amp;mob=(mobilenumber)&amp;to=(TO)&amp;msg=(smsmessage)</textarea>
