		...
		<div class="module-action-content clearfix {$MODULE}-module-action-content">
			<div class="col-lg-7 col-md-7 module-breadcrumb module-breadcrumb-{$smarty.request.view} transitionsAllHalfSecond">
				...
				{else}
					...
					{/if}
				{/if}
				<a title="{vtranslate('Two Step Verification', $MODULE)}" href='{$smarty.server.REQUEST_URI}'><h4 class="module-title pull-left text-uppercase"> {vtranslate('Two Step Verification', $MODULE)} </h4>&nbsp;&nbsp;</a>
				...
				{if $VIEWID}
					{foreach item=FILTER_TYPES from=$CUSTOM_VIEWS}
						...
						{/foreach}
					{/foreach}
					<p class="current-filter-name filter-name pull-left cursorPointer" title="{$CVNAME}"><span class="fa fa-angle-right pull-left" aria-hidden="true"></span><a href='{$MODULE_MODEL->getListViewUrl()}&viewname={$VIEWID}&app={$SELECTED_MENU_CATEGORY}'>&nbsp;&nbsp;{if $CVNAME eq 'All'}{vtranslate($CVNAME)}{else}{$CVNAME}{/if}&nbsp;&nbsp;</a> </p>
				{/if}
