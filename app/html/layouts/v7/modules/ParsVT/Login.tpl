    ...
    <div class="container-fluid loginPageContainer">
        <div class="col-lg-11 col-md-12 col-sm-12 col-xs-12">
            <div class="loginDiv widgetHeight">
                ...
                <a href="{if strpos($CompanyDetails['website'], "http") === false}//{/if}{$CompanyDetails['website']}"
                   target="_blank"><img class="img-responsive user-logo" title="{$CompanyDetails['companyname']}"
                                        src="test/logo/{$CompanyDetails['logoname']}"></a>

                <div>
					<span class="{if !$ERROR}hide{/if} failureMessage" id="validationMessage">
{vtranslate($MESSAGE, 'ParsVT')}</span>
...
{vtranslate($MESSAGE, 'ParsVT')}</span>
                ...
                <div id="loginFormDiv">
                    <form class="form-horizontal" method="POST" action="index.php">
                        ...
                        <div class="group">
                            <button type="submit" class="button buttonBlue">{vtranslate('Sign in', 'ParsVT')}</button>
                            <br>
                            <a class="forgotPasswordLink" style="color: #15c;">
                                {vtranslate('forgot password?', 'ParsVT')}</a>
                        </div>
                    </form>
                </div>
                ...
                <div id="forgotPasswordDiv" class="hide">
                    <form class="form-horizontal" action="forgotPassword.php" method="POST">
                        ...
                        <div class="group">
                            <input id="email" type="email" name="emailId" placeholder="{vtranslate('Email', 'ParsVT')}">
                            <span class="bar"></span>
                            <label>{vtranslate('Email', 'ParsVT')}</label>
                        </div>
                        <div class="group">
                            <button type="submit" class="button buttonBlue forgot-submit-btn">
                                {vtranslate('Submit', 'ParsVT')}
                            </button>
                            <br>

                            <span>{vtranslate('Please enter details and submit', 'ParsVT')}<a
                                        class="forgotPasswordLink {if vglobal('current_language') eq 'fa_ir'}pull-left{else}pull-right{/if}"
                                        style="color: #15c;">{vtranslate('Back', 'ParsVT')}</a></span>
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            loginFormDiv.find('button').on('click', function () {
                ...
                var errorMessage = '';
                if (username === '') {
                    errorMessage = '{vtranslate('Please enter valid username', 'ParsVT')}';
                    result = false;
                } else if (password === '') {
                    errorMessage = '{vtranslate('Please enter valid password', 'ParsVT')}';
                    result = false;
                }
                if (errorMessage) {
            ...
            forgotPasswordDiv.find('button').on('click', function () {
                ...
                if (username === '') {
                    ...
                    errorMessage = '{vtranslate('Please enter valid email address', 'ParsVT')}';
                    result = false;
                } else if (email.match(illegalChars)) {
                    errorMessage = '{vtranslate('The email address contains illegal characters.', 'ParsVT')}';
                    result = false;
                }
                if (errorMessage) {
                    validationMessage.removeClass('hide').text(errorMessage);
