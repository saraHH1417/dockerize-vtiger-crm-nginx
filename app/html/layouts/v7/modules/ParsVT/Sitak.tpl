                        ...
                        <tr {if $FIELD_NAME eq 'vtigersecretkey' or $FIELD_NAME eq 'webappurl'or $FIELD_NAME eq 'outboundcontext'or $FIELD_NAME eq 'outboundtrunk'}class="hide" {/if}>
                            ...
                            <td style="word-wrap:break-word;">
                                <input class="inputElement fieldValue"  style="direction: ltr; text-align: left; max-width: 300px" type="{$FIELD_VALUE[0]}"
                                       name="{$FIELD_NAME}" data-rule-required="true"  {if $FIELD_NAME eq 'sitak_url'} placeholder="http://YourSitakURL/connect-CRM/" {/if} value="{if $FIELD_NAME eq 'outboundcontext'or $FIELD_NAME eq 'outboundtrunk'}from-internal{else if $FIELD_NAME eq 'vtigersecretkey'}{if !empty($FIELD_VALUE[1])}{$FIELD_VALUE[1]}{else}{PBXManager_Server_Model::generateVtigerSecretKey()}{/if}{elseif $FIELD_NAME eq 'webappurl'}{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Sitak/{else}{$FIELD_VALUE[1]}{/if}"/>
                            </td>
                        </tr>
                    {/foreach}
                    <tr>
