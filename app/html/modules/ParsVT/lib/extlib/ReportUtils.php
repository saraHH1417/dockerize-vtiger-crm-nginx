...
function getReportFieldValue ($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false) {
	...
	} elseif ( $fieldInfo['uitype'] == 21 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_arf' ) {////BY Mahdi
		$fieldvalue = repeaterView( $fieldvalue );
	} elseif ( $fieldInfo['uitype'] == 53 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_atf' ) {////BY Mahdi
		$fieldvalue = getUserFullName( intval( $fieldvalue ) );
	} elseif ( $fieldInfo['uitype'] == 52 ) {////BY Mahdi
		$fieldvalue = is_numeric($fieldvalue) ? getUserFullName( intval( $fieldvalue ) ) : $fieldvalue;
	} elseif ( $fieldInfo['fieldname'] == 'callstatus' ) {////BY Mahdi
		$fieldvalue = vtranslate(  $fieldvalue ,'PBXManager'  );
	}  elseif ( $fieldInfo['fieldname'] == 'direction' ) {////BY Mahdi
		$fieldvalue = vtranslate(  $fieldvalue ,'PBXManager' );
	} elseif ( $fieldInfo['uitype'] == 780 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_cul' ) {////BY Mahdi
