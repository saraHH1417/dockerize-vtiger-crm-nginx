...
class PBXManager_Record_Model extends Vtiger_Record_Model
{
    ...
    public function searchIncomingCall(){
        if (vtlib_isModuleActive("ParsCTI") && file_exists("modules/ParsCTI/config.inc.php")) {
            include "modules/ParsCTI/config.inc.php";
        }
        if (!$popup_time) $popup_time = 45;
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            if($timeDiff > $popup_time && $rowData['callstatus'] == 'ringing') {
    ...
    public static function lookUpRelatedWithNumber($from, $callAssignedIserId = null)
    {
        ...
        //VTFarsi.ir begin fix search entity by number
        if ($fnumber == NULL) {
            return;
        }
