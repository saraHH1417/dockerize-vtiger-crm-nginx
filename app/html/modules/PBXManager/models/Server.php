...
class PBXManager_Server_Model extends Vtiger_Base_Model
{
    ...
    public static function getInstance()
    {
        ...
        if (vtlib_isModuleActive("Satel") && file_exists("modules/Satel/config.inc.php")) {
            include "modules/Satel/config.inc.php";
            $serverModel->set('gateway', "PBXManager");
            $serverModel->set('id', "0");
            $parameters = array('webappurl' => $site_URL . 'modules/Satel/', 'outboundcontext' => 'from-satel', 'vtigersecretkey' => self::generateVtigerSecretKey());
            foreach ($parameters as $fieldName => $fieldValue) {
                $serverModel->set($fieldName, $fieldValue);
            }
            return $serverModel;
        } else if (vtlib_isModuleActive("ParsCTI") && file_exists("modules/ParsCTI/config.inc.php")) {
            include "modules/ParsCTI/config.inc.php";
            $serverModel->set('gateway', "PBXManager");
            $serverModel->set('id', "0");
            $parameters = array('webappurl' => $site_URL . 'modules/ParsCTI/', 'outboundcontext' => 'from-parscti', 'vtigersecretkey' => self::generateVtigerSecretKey());
            foreach ($parameters as $fieldName => $fieldValue) {
                $serverModel->set($fieldName, $fieldValue);
            }
            return $serverModel;
        } else if ($gatewatResultCount > 0) {
    ...
    public static function checkPermissionForOutgoingCall()
    {
        ...
        if (vtlib_isModuleActive("ParsCTI") || vtlib_isModuleActive("Satel")) return false;
