...
class PBXManager_IncomingCallPoll_Action extends Vtiger_Action_Controller
{
    ...
    public function searchIncomingCalls(Vtiger_Request $request)
    {
        ...
        if (vtlib_isModuleActive("ParsCTI")) {
            if (file_exists("modules/ParsCTI/config.inc.php")) {
                include "modules/ParsCTI/config.inc.php";
            }
            $current_user_id = $user->id;
            $popupusers = explode(",", $popupusers);
            $groups = Users_Record_Model::getUserGroups($current_user_id);
            $groups = (empty($groups) || (is_null($groups)) ? array() : $groups);
            $groups = array_merge(array($current_user_id), $groups);
            $groupresult = array_intersect($groups, $popupusers);
            if (!empty($groupresult))
                $user_popup = true;
            else
                $user_popup = false;
        } elseif (vtlib_isModuleActive("ParsVT")) {
            $analogstatus = 'false';
            if (file_exists("modules/ParsVT/V2/PBX/config.php")) {
                include "modules/ParsVT/V2/PBX/config.php";
            }
            $analogstatus = ($analogstatus == 'true' || $analogstatus == 'Active' ? true : false);
            if ($analogstatus) {
                $current_user_id = $user->id;
                $popupusers = explode(",", $analogpopupusers);
                $groups = Users_Record_Model::getUserGroups($current_user_id);
                $groups = (empty($groups) || (is_null($groups)) ? array() : $groups);
                $groups = array_merge(array($current_user_id), $groups);
                $groupresult = array_intersect($groups, $popupusers);
        ...
        if ($recordModels) {
            foreach ($recordModels as $recordModel) {
                ...
                if($user->id != $recordModel->get('user') && !$user_popup ) {
                    continue;
                }
                ...
                if ($callerid && isRecordExists($callerid)) {
                    ...
                    if ($moduleName == 'Users') {
                        ...
                        $recordModel->set('callername', getUserFullName($callerid));
                        $recordModel->set('ownername', getUserFullName($userid));
                    } else {
                        $callerRecordModel = Vtiger_Record_Model::getInstanceById($callerid);
                        ...
                        $moduleName = $callerRecordModel->getModuleName();
                        $recordModel->set('customertype', $moduleName);
    ...
    public function createRecord(Vtiger_Request $request)
    {
        ...
        if ($request->get('lastname')) {
            $element['firstname'] = $request->get('firstname', null);
        ...
        if (function_exists('vtws_parsvt_create'))
            $entity = vtws_parsvt_create($moduleName, $element, $user);
        else
    ...
    function checkPermissionForPolling(Vtiger_Request $request)
    {
        ...
        $analogstatus = ($analogstatus == 'true' || $analogstatus == 'Active' ? true : false);
        if (vtlib_isModuleActive("ParsCTI") || vtlib_isModuleActive("Satel") || $analogstatus)
            $gateway = "PBXManager";
        else
