...
Settings_Vtiger_List_Js("Settings_ParsCPUsers_List_Js",{},{
    
    
	registerFilterChangeEvent : function() {
		var thisInstance = this;
		jQuery('#cpstatus').on('change',function(e){
		...
		jQuery('#searchusername').on('click',function(e){
			jQuery('#pageNumber').val("1");
			jQuery('#pageToJump').val('1');
			jQuery('#orderBy').val('');
			jQuery("#sortOrder").val('');
			var value = jQuery('#cpusername').val();
			
			var params = {
				module : app.getModuleName(),
				parent : app.getParentModuleName(),
				'search_key' : 'cpusername',
				'search_value' : value,
				'page' : 1,
			};
	...
	getDefaultParams : function() {
		...
		var params = {
			...
			'page' : pageNumber,
			'view' : "List",
			'user_name' : jQuery('select[id=cpstatus] option:selected').attr('name'),
			'search_key' : 'cpstatus',
			'search_value' : jQuery('#cpstatus').val()
		};
		...
		app.request.get({'data' : newListParams}).then(
			function(err, data){
				...
				if(err === null) {
					...
					vtUtils.showSelect2ElementView(jQuery('#cpstatus'));
