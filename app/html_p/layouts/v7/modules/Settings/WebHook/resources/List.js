 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_List_Js("Settings_WebHook_List_Js",{
	
	registerFilterChangeEvent : function() {
		var thisInstance = this;
		jQuery('#UserLists').on('change',function(e){
	...
	getDefaultParams : function() {
		...
		var params = {
			...
			'view' : "List",
			'search_key' : 'status',
			'search_value' : jQuery('#UserLists').val()
		};
		...
		app.request.get({'data' : newListParams}).then(
			function(err, data){
				...
				if(err === null) {
					...
					vtUtils.showSelect2ElementView(jQuery('#UserLists'));
	...
	pageJumpOnSubmit : function(element) {
		...
		var value = jQuery('#UserLists').val();
    ...
    showInformation : function() {
        $('.logcounter').click(function (event) {
                var id = $(this).attr('id');
				var url = 'index.php?module=WebHook&parent=Settings&view=ShowInfo&record='+id;
...
var AppTextArea = {
    init: function(){
        ...
        $trigger.click(function(){
            $value = $textarea.val();
            $customTextArea.addClass('show').find('.edit').focus().text($value);
        });

        $close.bind('blur click', function(){
            $value = $customTextArea.text();
            $customTextArea.removeClass('show');
