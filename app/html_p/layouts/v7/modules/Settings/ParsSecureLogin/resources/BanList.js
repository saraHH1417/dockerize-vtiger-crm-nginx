...
Settings_Vtiger_List_Js("Settings_ParsSecureLogin_BanList_Js",{},{

	registerFilterChangeEvent : function() {
		var thisInstance = this;

		
		jQuery('#searchparsbanlist').on('click',function(e){
			jQuery('#pageNumber').val("1");
			jQuery('#pageToJump').val('1');
			jQuery('#orderBy').val('');
			jQuery("#sortOrder").val('');
			var value = jQuery('#parsbanlist').val();
			
			var params = {
				module : app.getModuleName(),
				parent : app.getParentModuleName(),
				'search_key' : 'parsbanlist',
				'search_value' : value,
				'page' : 1,
			};
