...
Vtiger.Class("RelatedBlocksLists_Manager_Js",{
    ...
    registerEventForFilterValue:function(){
        ...
        $('body').delegate('#related_block_list_filter','change',function(e){
            ...
            $('#related_block_list_value').html('<option value="" >'+app.vtranslate('Select an option')+'</option>');
            if(arr.length > 0){
    ...
    changeSortPicklist : function(moduleName){
        ...
        $('#related_block_list_sortfield').html('<option value="" >'+app.vtranslate('None')+'</option>');
    ...
    changeFilterPicklist:function(moduleName){
        ...
        $('#related_block_list_filter').html('<option value="" >'+app.vtranslate('Select an option')+'</option>');
        var arrValue = {};
        if(arr !='' && arr != undefined){
            for(var key in arr) {
                var value = arr[key];
        ...
        app.request.post({data:params}).then(
            function (err,data) {
                if (err == null && data.enable == '1') {
                    ...
                    var relatedBlocksListsButtons=jQuery('<button style="margin-left:5px; margin-right:5px" data-url="index.php?module=RelatedBlocksLists&view=MassSettingsAjax&mode=showSettingsForm&type=block&sourceModule=' + sourceModule + '" type="button" class="btn addButton btnRelatedBlocksLists"><i class="fa fa-plus"></i>&nbsp; '+app.vtranslate("Add Related Block")+'</button><button style="margin-left:5px; margin-right:5px" data-url="index.php?module=RelatedBlocksLists&view=MassSettingsAjax&mode=showSettingsForm&type=list&sourceModule=' + sourceModule + '" type="button" class="btn addButton btnRelatedBlocksLists"><i class="fa fa-plus"></i>&nbsp; '+app.vtranslate("Add Related List")+'</button>');
            ...
            function (err,data) {
                if (err == null && data.enable == '1') {
                    ...
                    if(addCustomBlockButton.closest('#detailViewLayout').find('.btnRelatedBlocksLists').length == 0) {
                        var relatedBlocksListsButtons = jQuery('<button style="margin-left:5px; margin-right:5px" data-url="index.php?module=RelatedBlocksLists&view=MassSettingsAjax&mode=showSettingsForm&type=block&sourceModule=' + sourceModule + '" type="button" class="btn addButton btnRelatedBlocksLists"><i class="icon-plus"></i><strong>'+app.vtranslate("Add Related Block")+'</strong></button><button style="margin-left:5px; margin-right:5px" data-url="index.php?module=RelatedBlocksLists&view=MassSettingsAjax&mode=showSettingsForm&type=list&sourceModule=' + sourceModule + '" type="button" class="btn addButton btnRelatedBlocksLists"><i class="icon-plus"></i><strong>'+app.vtranslate("Add Related List")+'</strong></button>');
                        addCustomBlockButton.after(relatedBlocksListsButtons);
