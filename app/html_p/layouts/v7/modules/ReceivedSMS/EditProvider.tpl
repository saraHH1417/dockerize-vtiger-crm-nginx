         ...
         <div class="listViewContentDiv" id="listViewContents">
            <div class="listViewEntriesDiv">
               <form class="form-horizontal" id="smsConfig" method="POST">
				...
				<select data-fieldtype="picklist" class="inputElement select2  select2-offscreen providerType" type="picklist" style="width:200px" name="Provider" id="Provider" data-validation-engine="validate[ required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]">
							<option></option>
                        ...
                        <td class="row-fluid medium"><select  data-fieldtype="picklist" class="inputElement select2  select2-offscreen" type="picklist" style="width:200px" name="status">{$Status}</select></td>
                        </tr>
                        </tbody>
                        </table>
			...
			{assign var=counter value=0} 					
	         	{foreach key=PROVIDER_NAME item=PROVIDER_DETAILS from=$providerlist}
                        ...
                        <td class="row-fluid medium">
			<input type="{if $ENTRY eq 'password'}password{else}text{/if}" name="{$PROVIDER_NAME}[{$ENTRY}]"  class="form-control" style="width:200px" data-validation-engine="validate[required]" value="{$serverparams[$PROVIDER_NAME][$counter]}" />
                        </td>
                        </tr>
