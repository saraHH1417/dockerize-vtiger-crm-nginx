        ...
        autoFillData: function(container,mapping, arrFields) {
            ...
            for (felement in mapping) {
                if(container.find('[name="'+felement+'"]').length>0) {
                    ...
                    else if(!container.find('[name^="'+felement+'"]').is('select.select2') && container.find('[data-name^="'+felement+'"]').is('select.select2')) {

                        var values=mapping[felement].split(",");
                        jQuery.each(values, function(i,e){
                            container.find('[data-name^="'+felement+'"] option[value="'+e+'"]').prop("selected", true);
                        });
                        container.find('[data-name^="'+felement+'"]').trigger("change");
                    }
                    ...
                    if(jQuery.inArray(felement,arrFields) !== -1 ) {
						...
						if(mapping[refield] == 0) {
                            var decoded = $("<textarea/>").html("").text();
                            container.find('[name="' + refield + '_display"]').val(decoded).attr('readonly', true);
                        }
						else{
                            var actionParams = {
                                "type": "POST",
...
function sprintformat() {
    var a, b, c;
    a = app.vtranslate(arguments[0]);
    b = [];
                                    ...
                                    function(err,data){
                                        if(err === null) {
                                            if (data != null) {
                                                ...
                                                if (showPopup == 1) {
                                                    var message = sprintformat('Overwrite the existing fields with the selected %s %s details',  moduleLabel , ' (' + selectedName + ') ');
