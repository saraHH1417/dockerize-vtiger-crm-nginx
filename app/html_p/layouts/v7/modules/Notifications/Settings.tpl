...
<div class="container-fluid">
    ...
    <div class="summaryWidgetContainer">
        <div class="row-fluid">
	<div class="row">
		<label><input type="checkbox" name="enable_module" id="enable_module" value="1" {if $ENABLE eq '1'}checked="" {/if}/> {vtranslate('LBL_ENABLE_MODULE', 'Notifications')}</label>
	</div>
        	<div class="row">
	            <div class="col-md-1" style="width: 13%">
	                <span class="span2"><h>{vtranslate('Sort by :', 'Notifications')}</h4></span>
	            </div>
	            <div class="col-md-2">
	                <select name="sort_by" id="sort_by"  data-fieldtype="picklist" class="inputElement select2" type="picklist">
	                    <option value="ASC" {if $SORT_BY eq 'ASC'}selected{/if}>{vtranslate('Oldest', 'Notifications')}</option>
	                    <option value="DESC" {if $SORT_BY eq 'DESC'}selected{/if}>{vtranslate('Latest', 'Notifications')}</option>
	                </select>
	            </div>
        	</div>
        </div>
