...
-->*}
{strip}
    <div id="sendfaxContainer" class='modelContainer' style='max-width:calc(100vw - 400px);' data-max="{$MAXPDF}">
        <div class="modal-header contentsBackground">
            <button data-dismiss="modal" class="close" title="{vtranslate('LBL_CLOSE')}">&times;</button>
            <h3>{vtranslate('Send Fax to the selected numbers', $MODULE)}</h3>
        </div>
        ...
        <form class="form-horizontal" id="SendFax" method="post" action="index.php" enctype="multipart/form-data">
            ...
            <div class="modal-body tabbable slimScrollDiv"
                 style="position: relative; overflow: hidden; overflow-y: scroll; width: auto; height:auto; max-height: calc(100vh - 210px);">
                <table class="massEditTable table table-bordered">
                    ...
                    <tr>
                        ...
                        <td class="fieldValue">
                            <select id="providerName" class="chzn-select" name="provider" data-validation-engine='validate[required]'>
                        ...
                        {vtranslate('Browse',$MODULE)}… <input id="FaxFiles" type="file" name="file[]" accept=".pdf"
                            ...
                            <div id="attachments" class="row-fluid">
                                {foreach item=ATTACHMENT from=$ATTACHMENTS}
                                    ...
                                    {/if}
                                    <div class="MultiFile-label customAttachment" data-file-id="{$ATTACHMENT['fileid']}"
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-success" type="submit" name="saveButton">
                    <strong>{vtranslate('Send', $MODULE)}</strong></button>
            </div>
        </form>
    </div>
{/strip}
