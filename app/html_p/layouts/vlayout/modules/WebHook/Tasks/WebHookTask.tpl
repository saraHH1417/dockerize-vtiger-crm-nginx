...
<div class="row-fluid">
    ...
    <div  id="urlfields"  class="span2">
        <select class="chzn-select task-fields">
            {$ALL_FIELD_OPTIONS}
        </select>
    </div>
    <div class="span8">
        <input type="text" name="webhook_url" id="webhook_url" value="{$TASK_OBJECT->webhook_url}" data-validation-engine='validate[required]' data-rule-required="true" class="inputElement url fields" placeholder="{vtranslate('Enter Request Url',$WHMODULE)}" style="width:{if $TASK_OBJECT->content_type neq 'SOAP'}99{else}65{/if}%; direction:ltr; text-align:left;"><input id="getfunctionurls" class="btn" style="width:200px;{if $TASK_OBJECT->content_type neq 'SOAP'}display:none;{/if}" value="{vtranslate('Get list of WSDL url functions', $WHMODULE)}">
    ...
    <div class="row-fluid  padding-bottom1per" id="display_soap_version" {if $TASK_OBJECT->content_type neq 'SOAP'}style="display:none"{/if}>
        <div class="span2">{vtranslate('Soap Version',$WHMODULE)}<span class="redColor">*</span></div>
        <div class="span10">
            <select class="task-fields" name="soap_version" data-validation-engine='validate[required]' id="soap_version" style="width:30%;height:31px;">
