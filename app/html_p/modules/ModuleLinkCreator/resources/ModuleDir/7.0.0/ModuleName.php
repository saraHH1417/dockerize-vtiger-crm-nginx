...
class __ModuleName__ extends Vtiger_CRMEntity {
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			$this->addRelationtoNotifications();
    ...
    public function addRelationtoNotifications() {
        $classname = get_class($this);
        $module = explode("_", $classname);
        $module = $module[0];
        include_once "vtlib/Vtiger/Module.php";
        global $adb;
        $moduleNames = array($module);
        $moduleInstance = Vtiger_Module::getInstance('Notifications');
        if ($moduleInstance) {
            foreach ($moduleNames as $moduleName) {
                $relatedModuleInstance = Vtiger_Module::getInstance($moduleName);
                $relatedModuleInstance->setRelatedList($moduleInstance, 'Notifications', array("ADD"), "get_dependents_list");
            }
            $adb->pquery("UPDATE vtiger_relatedlists SET presence=1 WHERE related_tabid= ?", array($moduleInstance->getId()));
            $fieldModel = Vtiger_Field::getInstance("related_to", $moduleInstance);
            if ($fieldModel) $fieldModel->setRelatedModules($moduleNames);
        }
    }
}